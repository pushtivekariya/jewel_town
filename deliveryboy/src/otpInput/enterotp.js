...
import { verifyOTPApi } from "../api/common_api";
...
const Enterotp = () => {
  ...
  const verifyOtpFunc = async () => {
    if (otp.otp == "") {
      ...
      const response = await verifyOTPApi(otp.otp, state.email);
  ...
  return (
    <>
      <section className="container section" style={{ width: "30%" }}>
        ...
        <p className="para">
          VERIFICATION CODE ARE SENT TO THE USER'S EMAIL ADDRESS{" "}
        </p>
        <div id="inputs">
          <input
            className="inputs"
        ...
        <button className="btns" onClick={() => verifyOtpFunc()}>
          Submit
        </button>
        <button
          className="btnback"
