...
const CancelledOrder = () => {
  ...
  const cancelorderDataFunc = async () => {
    const response = await cencelledorderData();
    setcancelorderData(response.result);
  ...
  const submitFunction = (data) => {
    ...
    const options = {
      ...
      handler: async function (response) {
        const responses = await refundPaymentApi(data);
...

    {
      ...
      cell: (row) => (
        <Link
          ...
          value="update"
          onClick={() => {
            // setcancelorderData(row)
            // setInterval(submitFunction(row), 2000);
            submitFunction(row);
          }}
