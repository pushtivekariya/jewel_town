...
import { dispachedOrderApi, orderDetailsApi } from "../../api/common_api";
import DataTable from "react-data-table-component";
import { useState } from "react";
import { useEffect } from "react";

const DispatchedOrder = () => {
  const [dispatchedOrderData, setdispatchedOrderData] = useState([]);
  const [orderDetails, setorderDetails] = useState([]);
  const dispatchedOrderFunc = async (order_id) => {
    const response = await dispachedOrderApi(order_id);
    setdispatchedOrderData(response.result);
    console.log(dispatchedOrderData, "dispatchedOrderData");
  };
